...
RESOURCE_UCAT = "4318d38a-0bcd-421d-afb1-fb88b0c92a87"  # UCAT PJ (Alta tensão)
RESOURCE_UCMT = "f6671cba-f269-42ef-8eb3-62cb3bfa0b98"  # UCMT PJ (Média tensão)
...
def calc_score(df: pd.DataFrame) -> pd.DataFrame:
    ...
    - Urbanidade/acionável: 0–10 pontos (endereço mais completo tende a ser mais acionável)
    """
    import numpy as np
    ...
    # força numérico (evita dtype object)
...
def enrich_base(df: pd.DataFrame, demand_col: str, lat_col: str, lon_col: str, mun_col: str | None) -> pd.DataFrame:
    ...
    # UF via IBGE município (se houver)
    ...
    # Detecta colunas úteis (se existirem)
    ...
    def addr_text(r):
        parts = []
        if LGRD and pd.notna(r.get(LGRD)):
            parts.append(str(r.get(LGRD)).strip())
        if BRR and pd.notna(r.get(BRR)):
            parts.append(str(r.get(BRR)).strip())
        if CEP and pd.notna(r.get(CEP)):
            parts.append(f"CEP {str(r.get(CEP)).strip()}")
...
mun_col = st.sidebar.selectbox(
    "Coluna de Município (IBGE) (opcional)",
    ["(nenhuma)"] + cols,
...
pot_sel = st.sidebar.multiselect("Potencial (selecionar)", ["AAA", "AA", "A", "B", "C"], default=["AAA", "AA", "A"])
...
if btn_run:
    ...
    st.caption("Dica: para TOP grandes, aumente max_chunks e use filtro UF/potencial para manter rápido.")
